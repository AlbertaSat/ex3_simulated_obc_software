...
DeployablesService ::DeployablesService(const char *const compName)
    : DeployablesServiceComponentBase(compName) {
    // Initialize deployables map with all deployable items set to stowed
    // TODO - this will have to fetch the deployables deployment state from some
    // sort of non volatile memory
    ...
    LEOPDeploymentSequence = {"DFGM",  "UHF_P",   "UHF_Z",  "UHF_N",
                              "UHF_S", "SOLAR_S", "SOLAR_P"};
}

DeployablesService ::~DeployablesService() {}
...
void DeployablesService ::updateDeployableStatus(
    const std::string &deployableName, DeployableStatus status) {
...
void DeployablesService ::updateDeployableAttempts(
    const std::string &deployableName, int numAttempts) {
...
void DeployablesService ::attemptDeployment(const std::string &deployableName) {
    ...
    // Send command to toggle target component burnwire HIGH
    this->deployableToggleBurnwirePort_out(0, burnwireSetString.c_str());

    while (deployableInfo.numAttempts < NUMBER_OF_DEPLOYMENT_ATTEMPTS &&
           deployableInfo.status != DeployableStatus::DEPLOYED) {
        ...
        // Build string to send query for target components feedback switch
        // status
        std::string switchStatusRequestString =
            "switch_request:" + deployableName;

        // Send command to query target component feedback switch status
        this->deployableToggleBurnwirePort_out(
            0, switchStatusRequestString.c_str());

        // Wait for response to come in from deployables interface
        sleep(SWITCH_QUERY_RESPONSE_WAIT_TIME_SECONDS);

        updateDeployableAttempts(deployableName,
                                 deployableInfo.numAttempts + 1);

        // If N attempts fail, then this component is skipped
        if (deployableInfo.numAttempts >= NUMBER_OF_DEPLOYMENT_ATTEMPTS) {
            updateDeployableStatus(deployableName,
                                   DeployableStatus::FAILED_ALL_ATTEMPTS);
        }
...
// This handler is called when a deployable feedback switch status is received
// from the deployables interface (via async input port)
// TODO - error handling and checking response string
void DeployablesService ::deployableFeedbackSwitchPort_handler(
    ...
    // Parse the response string to get the deployable name and feedback switch
    // status
    std::string delimiter = ":";
    std::string deployableName =
        responseString.substr(0, responseString.find(delimiter));
    ...
    std::string feedbackSwitchStatus = responseString.substr(
        responseString.find(delimiter) + 1, responseString.length());
...
void DeployablesService ::ActivateDeploymentSequence_cmdHandler(
    const FwOpcodeType opCode, const U32 cmdSeq) {
    this->log_ACTIVITY_HI_deploymentSequenceStarted();
    // Loop through all items in deployables map and attempt to deploy them
    for (const std::string &deployableName : LEOPDeploymentSequence) {
        attemptDeployment(deployableName);
    }
    this->log_ACTIVITY_HI_deploymentSequenceEnded();
